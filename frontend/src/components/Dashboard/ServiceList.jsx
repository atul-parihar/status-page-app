...
const ServiceList = ({ services, setServicesData }) => {
  ...
  const handleServiceUpdate = async (e) => {
    ...
    try {
      const response = await fetch(`http://localhost:10000/api/services/${serviceToUpdate._id}`, {
        method: 'PUT',
        headers: {
  ...
  const handleServiceDelete = async (serviceId) => {
    ...
    try {
      const response = await fetch(`http://localhost:10000/api/services/${serviceId}`, {
        method: "DELETE",
        headers: {
          "Content-Type": "application/json",
        },
