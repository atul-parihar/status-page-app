...
const IncidentList = ({ incidents, onDelete, setIncidentsData }) => {
  ...
  const handleIncidentUpdate = async (e) => {
    ...
    try {
      const response = await fetch(`${process.env.REACT_APP_BACKEND_URL}/api/incidents/${incidentToUpdate._id}`, {
        method: 'PUT',
        headers: {
  ...
  const handleIncidentDelete = async (incidentId) => {
    ...
    try {
      const response = await fetch(`${process.env.REACT_APP_BACKEND_URL}/api/incidents/${incidentId}`, {
        method: "DELETE",
        headers: {
          "Content-Type": "application/json",
        },
